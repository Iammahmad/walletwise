...
export function transactionsToCsv(transactions: Transaction[]): string {
  const headers = ['id', 'type', 'amount', 'currency', 'merchant', 'category', 'account', 'occurred_at', 'note', 'source', 'original_transcript'];
...
export async function exportTransactionsCsv(transactions: Transaction[]): Promise<void> {
  ...
  await Sharing.shareAsync(file.uri, { mimeType: 'text/csv', dialogTitle: 'Export SpendSpeak transactions', UTI: 'public.comma-separated-values-text' });
}
