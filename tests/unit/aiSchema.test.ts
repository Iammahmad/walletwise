...
describe("AI output contract", () => {
  ...
  it("allows unknown financial fields to be null", () =>
    expect(
  ...
  it("rejects numbers in place of decimal strings", () =>
    expect(
      aiOutputSchema.safeParse({
        ...valid,
        transactions: [{ ...valid.transactions[0], amount: 1250 }],
      }).success,
    ).toBe(false));
  it("rejects unreviewed responses, bad confidence, and extra keys", () => {
    expect(
      aiOutputSchema.safeParse({ ...valid, needsConfirmation: false }).success,
    ).toBe(false);
    expect(
      aiOutputSchema.safeParse({
        ...valid,
        transactions: [{ ...valid.transactions[0], confidence: 1.2 }],
      }).success,
    ).toBe(false);
    expect(
      aiOutputSchema.safeParse({ ...valid, rawModelText: "do not trust me" })
        .success,
    ).toBe(false);
  });
});
