...
export default function ReviewScreen() {
  ...
  if (!draft)
    return (
      <Screen>
        <FeedbackState
          kind="empty"
          title="No voice draft"
          message="Nothing was saved. Start a new voice entry when you are ready."
  ...
  return (
    <Screen
      title={
        drafts.length > 1
          ? `Review entry ${index + 1} of ${drafts.length}`
          : "Review voice entry"
      }
      subtitle="Nothing is saved until you confirm this form."
    >
      {missing.length ? (
        <Card style={{ backgroundColor: colors.warningSoft }}>
          ...
          <Text style={[styles.warningBody, { color: colors.text }]}>
            These fields were missing or interpreted with low confidence.
          </Text>
        </Card>
      ) : (
        <Card style={{ backgroundColor: colors.primarySoft }}>
          ...
          <Text style={[styles.warningBody, { color: colors.text }]}>
            Check every field, then confirm explicitly.
          </Text>
        </Card>
      )}
      <TransactionForm
        ...
        preset={{
          ...
          budgetCategoryName: draft.budget ?? "",
...
const styles = StyleSheet.create({
  ...
  warningBody: { fontSize: 13, lineHeight: 19, marginTop: spacing.xs },
});
